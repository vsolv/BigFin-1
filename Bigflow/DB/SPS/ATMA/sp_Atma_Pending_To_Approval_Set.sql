CREATE  PROCEDURE `sp_Atma_Pending_To_Approval_Set`(in li_Action  varchar(30),
in lj_filter json,in lj_classification json,out Message varchar(1000))
sp_Atma_Pending_To_Approval_Set:BEGIN#####MOVE TO MST TABLE SP
...
/*
###renewal process
select partner_requestfor from atma_tmp_tpartner where
partner_gid=@Partner_Gid and partner_isactive='Y' into @requestfor;
#select @requestfor;
   if @requestfor='RENEWAL PROCESS' then
    SET SQL_SAFE_UPDATES = 0;
	update atma_tmp_tpartner set partner_status= @Partner_Status
...
partner_group,partner_custcategorygid,partner_compositevendor,
...
A.partner_compositevendor=B.partner_compositevendor,
...
set Message = 'FAILED UPDATE_PAR1';
rollback;
        leave sp_Atma_Pending_To_Approval_Set;
end if;
...
if @Main_partner_gid = '' or @Main_partner_gid is null  or @Main_partner_gid=0 then
    ...
    select @Out_Msg_SupplierGROUP,'group';
	if @Out_Msg_SupplierGROUP >0 then
		set @Msg_SupplierGROUP='SUCCESS';
        #select @Msg_SupplierGROUP'su';
	else
		set @Msg_SupplierGROUP='FAIL';
        rollback;
	end if;
else
###UPDATE SUPPLIER GROUP
  ...
  /*  if @Partner_Status='DEACTIVATION' OR @Partner_Status='TERMINATION' then
    update gal_mst_tsuppliergroup set suppliergroup_isactive='N',suppliergroup_isremoved='Y' where
	suppliergroup_isactive='Y'and suppliergroup_isremoved='N' and suppliergroup_gid=@supp_grp_gid;
    end if;
    if @Partner_Status='ACTIVATION' then
    update gal_mst_tsuppliergroup set suppliergroup_isactive='Y',suppliergroup_isremoved='N' where
	suppliergroup_isactive='N'and suppliergroup_isremoved='Y' and suppliergroup_gid=@supp_grp_gid;
    end if;*/

    #####end
	set @lj_supplier= concat('{"SupplierGrp_Name":"',@partner_name,'"
    ,"AtmaSupplierGrp_Code":"',@partner_code,'",
    "Supplier_Capacity":0,"SupplierGrp_Gid":',@supp_grp_gid,'}');
    #select @lj_supplier,'sup_grp update';
	set @lj_address=concat('{"supplier_add_gid":',@Address_Gid,'}');
	set @lj_contact=concat('{"supplier_contact_gid":',@Contact_Gid,'}');
	#select @lj_supplier,@lj_address,@lj_contact,1;
	call sp_SupplierSPS_Set('SUPPLIER_GROUP_UPDATE',
    @lj_supplier,@lj_address,@lj_contact,lj_Classification,@Message);
	select @Message into @Msg_SupplierGROUP ;
    #select @Out_Msg_SupplierGROUP;
	#if @Out_Msg_SupplierGROUP ='SUCCESS' then
...
if @Msg_SupplierGROUP='SUCCESS' then

if @Out_Msg_SupplierGROUP is null then#### new insert supplier
set @Out_Msg_SupplierGROUP=@supp_grp_gid;
end if;
	set @Supplier_msg ='';
	SET finished1 =0;
	BEGIN
	Declare Cursor_atma1 CURSOR FOR
		...
		select partnerbranch_gid,partnerbranch_gstno,partnerbranch_panno,partnerbranch_addressgid,partnerbranch_contactgid,
        partnerbranch_name,partnerbranch_creditperiod,partnerbranch_creditlimit,partnerbranch_paymentterms,
        partnerbranch_code
	...
	if finished1 = 1 then
		leave atma_looop1;
	End if;
	select fn_supplier(p_partnerbranch_code) into @up_supp_gid ;

if @up_supp_gid =0  then
	...
	select @Supplier_partner,'insert';
else
	#select p_partnerbranch_code,'code';
    #select fn_supplier(p_partnerbranch_code) into @up_supp_gid ;

   ####supplier Deactivation and Activation
   select suppliergroup_gid from gal_mst_tsuppliergroup where
	suppliergroup_code=@partner_code into @supp_grp_gid;
    if @Partner_Status='DEACTIVATION' OR @Partner_Status='TERMINATION' then
   # select 1;
    SET SQL_SAFE_UPDATES=0;
	update gal_mst_tsupplier set supplier_isactive='N',supplier_isremoved='Y' where
	supplier_isactive='Y'and supplier_isremoved='N' and supplier_groupgid=@supp_grp_gid;
    #select @Partner_Status='DEACTIVATION',@supp_grp_gid;
	end if;
	if @Partner_Status='ACTIVATION' then
    SET SQL_SAFE_UPDATES=0;
   # select @Partner_Status,'activation';
    update gal_mst_tsupplier set supplier_isactive='Y',supplier_isremoved='N' where
	supplier_isactive='N'and supplier_isremoved='Y' and supplier_groupgid=@supp_grp_gid;
    end if;
    ####end supplier Deactivation and Activation
    set @lj_supplier= concat('{"Supplier_Gid":',@up_supp_gid,',
		"Supplier_Name":"',@partner_name,'",
		"Supplier_Capacity":0,
        "Supplier_Group_Gid":',@supp_grp_gid,',
		"atmaSupplier_Code":"',p_partnerbranch_code,'",
		"Supplier_Branch_Name":"',p_partnerbranch_name,'",
        "Supplier_GST_No":"',ifnull(p_partnerbranch_gstno,''),'",
		"Supplier_Panno":"',ifnull(p_partnerbranch_panno,''),'"}');
		set @lj_address=concat('{"supplier_add_gid":',p_partnerbranch_addressgid,'}');
		set @lj_contact=concat('{"supplier_contact_gid":',p_partnerbranch_contactgid,'}');

	call sp_SupplierSPS_Set('SUPP_UPDATE',@lj_supplier,@lj_address,@lj_contact,lj_Classification,@Message);
	select @Message into @Supplier_partner ;

end if;
	SET @suppliergid = (SELECT SPLIT_STR((@Supplier_partner), ',', 2));
	SET @Supplier_msg = (SELECT SPLIT_STR((@Supplier_partner), ',', 1));
	select @Supplier_msg,'insert';
	#SET @suppliergid=(select SUBSTRING_INDEX(@Supplier_partner,',',-1));
	#SET @Supplier_msg=(select SUBSTRING_INDEX(@Supplier_partner,',',1));
	if  @Supplier_msg = 'SUCCESS' or @Supplier_partner= 'SUCCESS' then
    select @Supplier_msg,'check';
  ####update map_table
  if @suppliergid is null or @suppliergid =''then
  set @suppliergid=@up_supp_gid;
  #else
  #set @sup_id=@suppliergid;
  end if;

				SET SQL_SAFE_UPDATES = 0;
				Update gal_map_tsupplierproduct
				set  supplierproduct_isactive= 'N' ,supplierproduct_isremoved='Y'
				Where supplierproduct_supplier_gid =@suppliergid;
		...
		set @partner_ref_gid_supp_payment='';
			...
			select fn_bankdetails(p_partnerbranch_code) into @bankdetailsgid ;

  ###gal_trn_tbankdetails
 if @bankdetailsgid =0 then
		set Query_Update='';
		...
		set Query_Update = concat('Update gal_trn_tbankdetails
									set  bankdetails_paymode_gid=',p_payment_paymodegid,',
                                    bankdetails_bank_gid=''',p_payment_bankgid,''',
									bankdetails_bankbranch_gid=''',p_payment_bankbranchgid,''',
                                    bankdetails_acno=''',ifnull(p_payment_accnumber,''),''',
									bankdetails_beneficiaryname=''',ifnull(p_payment_benificiaryname,''),''' ,
                                    update_by=''',@Update_By,''', Update_date = now()
									Where bankdetails_gid =',@bankdetailsgid,';

                                            ');

end if;###tbankdetails @up_supp_gid
              #select Query_Update,'update,bankdetails';
        set @Insert_query = Query_Update;
		PREPARE stmt FROM @Insert_query;
		EXECUTE stmt;
		set countRow = (select ROW_COUNT());
        ...
        select fn_creditlimit(p_partnerbranch_code) into @creditlimitgid;

 if @creditlimitgid =0 then
		set Query_Update='';
		set Query_Update = concat('INSERT INTO gal_trn_tcreditlimit (creditlimit_ref_gid,creditlimit_reftable_gid,
			...
			values (',@partner_ref_gid_supp_payment,',',@suppliergid,',
					...
					',p_entity_gid,',',p_create_by,',
					''',Now(),''')');
                    #select Query_Update,'credit';

else
			set Query_Update = concat('Update gal_trn_tcreditlimit
				set  creditlimit_limit=''',p_partnerbranch_creditlimit,''',
                creditlimit_days= ''',p_partnerbranch_creditperiod,''',
                creditlimit_creditterms= ''',ifnull(p_partnerbranch_paymentterms,''),''',
                creditlimit_validfrom=',if(ifnull(@partner_contractdatefrom,null) IS NULL,'NULL',CONCAT('''',@partner_contractdatefrom,'''')),' ,
                creditlimit_validto=',if(ifnull(@partner_contractdateto,null) IS NULL,'NULL',CONCAT('''',@partner_contractdateto,'''')),' ,
				update_by=''',@Update_By,''', Update_date = now()
                Where creditlimit_gid =',@creditlimitgid,'
...
###@creditlimit_gid
		...
		#select Query_Update,'update creditlimit';
        set @Insert_query = Query_Update;
		PREPARE stmt FROM @Insert_query;
		EXECUTE stmt;
		set countRow = (select ROW_COUNT());
...
else #if @out_msg_suppgroup='SUCCESS' then
	rollback;
	leave sp_Atma_Pending_To_Approval_Set;
end if;#if @out_msg_suppgroup='SUCCESS' then
#select @Msg_SupplierGROUP,@Supplier_msg;
if @Msg_SupplierGROUP='SUCCESS' and @Supplier_msg = 'SUCCESS' then
	#rollback;
	commit;
else
	rollback;
