...
BEGIN
	Declare Cursor_atma CURSOR FOR
			...
			entity_gid,create_by,update_by,main_activity_gid
	...
	atma_looop:loop
		...
		fetch Cursor_atma into a_activity_gid,a_activity_partnergid,a_activity_code,a_activity_partnercode,a_activity_partnerbranchgid,a_activity_type,a_activity_name,
			a_activity_desc,a_activity_startdate,a_activity_enddate,a_activity_projectedspend,a_activity_rm,a_activity_contactgid,
			a_activity_fidiinsur,a_activity_bidding,a_activity_reason,a_activity_status,a_activity_isactive,a_activity_isremoved,
			a_entity_gid,a_create_by,a_update_by,a_main_activity_gid;
   ...
   set Error_Level='ATMA49.1';
			...
			set Query_Update = CONCAT('INSERT INTO gal_mst_tcontact (Contact_ref_gid,Contact_reftable_gid,contact_reftablecode,Contact_contacttype_gid,Contact_personname,Contact_designation_gid,
			Contact_landline,Contact_landline2,Contact_mobileno,Contact_mobileno2,Contact_email,Contact_DOB,Contact_WD,entity_gid,create_by)values
            (',@Contact_ref_gid,',
...
if a_main_activity_gid = '' or a_main_activity_gid is null  or a_main_activity_gid=0 then
    ...
    (',Partner_Gid,',''',a_activity_code,''',''',a_activity_partnercode,''',
			...
			#select @newpartnerbranchgid,@oldpartnerbranchgid;
           #select Query_Update;
            #select Partner_Gid,a_activity_code,a_activity_partnercode,a_activity_type,a_activity_reason;
            #select Query_Update;
            SET SQL_SAFE_UPDATES = 0;
			...
			activity_contactgid=',a_activity_contactgid,',
...
Declare Cursor_atma1 CURSOR FOR
	...
	if ad_main_activitydetails_gid = '' or ad_main_activitydetails_gid is null  or ad_main_activitydetails_gid=0 then
			...
			set Query_Update = concat('INSERT INTO atma_mst_tactivitydetails (activitydetails_activitygid,activitydetails_code,activitydetails_name,activitydetails_raisor,
			activitydetails_approver,activitydetails_isactive,activitydetails_isremoved,entity_gid,create_by',Query_Column,')
