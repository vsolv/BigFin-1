...
IF ls_Action='Checker_Get' then
        ...
        if @Partner_Gid is not null or @Partner_Gid <> '' then
			set Query_Search = concat(Query_Search,' and pa.partner_gid = ',@Partner_Gid,' ');
		End if;
        ...
        em.employee_name,branch.partnerbranch_name
        from  atma_map_tpartnerproduct mp
        ...
        inner join gal_mst_temployee em on em.employee_gid=mp.create_by
        inner join atma_mst_tpartnerbranch branch on branch.partnerbranch_gid=mp.mpartnerproduct_partnerbranch_gid
        where mp.entity_gid=',@Entity_Gid ,' and pa.partner_isactive=''Y''
...
IF ls_Action='PENDING_REJECTED' then
        ...
        and pr.entity_gid=',@Entity_Gid ,' and em.employee_isactive=''Y''
        and em.employee_isremoved=''N''and em.entity_gid=',@Entity_Gid ,'
        and mp.mpartnerproduct_status in(''PENDING'',''REJECTED'') ',Query_Search,'
                                    ');
                #select Query_Select;
     set @p = Query_Select;
    #select Query_Select;  ## Remove It
