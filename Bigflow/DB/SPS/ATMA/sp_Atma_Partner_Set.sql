CREATE  PROCEDURE `sp_Atma_Partner_Set`(in ls_Action varchar(160),
...
if ls_Action = 'INSERT'  then
	...
	if @li_jsonpartner is not null or @li_jsonpartner <> '' then
		 ...
		 select JSON_UNQUOTE(JSON_EXTRACT(partner_json,CONCAT('$.Partner_Compositevendor')))into @Partner_Compositevendor;
   ...
   /* select @Partner_Compositevendor;
    if @Partner_Compositevendor='NO' then
   # select @Partner_Compositevendor;
		set @PartnerCompositevendor='N';
	elseif @Partner_Compositevendor='YES' then
		set @PartnerCompositevendor='Y';
	end if;*/
...
#set @AT_partnercode='';
	...
	(partner_code,partner_name,partner_panno,partner_compregno,partner_group,partner_custcategorygid,partner_compositevendor,
	...
	(''',@code_partner,''',''',@Partner_Name,''',''',ifnull(@Partner_Panno,''),''',''',ifnull(@Partner_Compregno,''),''',''',@Partner_Group,''',
...
#####partner_page
				set Query_Update1='';
					...
					End if;
                     ...
                     if @Partner_Compositevendor  <>'null' and @Partner_Compositevendor <> '' then

						set Query_Update1 = concat(Query_Update1,',partner_compositevendor = ''',@Partner_Compositevendor,'''  ');
                        #select @PartnerCompositevendor;
					else
						set Message='Please Enter compositevendor' ;
						leave sp_Atma_Partner_Set;
					End if;
